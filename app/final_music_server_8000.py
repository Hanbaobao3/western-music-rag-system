...
class FinalRAGHandler(BaseHTTPRequestHandler):
    ...
    def identify_period_ultimate(self, question):
        ...
        period_scores['ancient'] = sum(1 for kw in ancient_keywords if kw in question_lower)

        # 中世纪
        medieval_keywords = ['中世纪', '格里高利', '圣咏', '基督教', '奥尔加农', '经文歌', '等节奏', '教会调式', '弥撒', '礼拜']
        period_scores['medieval'] = sum(1 for kw in medieval_keywords if kw in question_lower)

        # 文艺复兴
        renaissance_keywords = ['文艺复兴', '迪法伊', '勃艮第', '奥克冈', '帕莱斯特里那', '世俗', '人文主义', '印刷术', '牧歌', '尚松', '利德']
        period_scores['renaissance'] = sum(1 for kw in renaissance_keywords if kw in question_lower)

        # 巴洛克
        baroque_keywords = ['巴洛克', '佩里', '蒙泰韦尔迪', '吕利', '拉莫', '珀塞尔', '科雷利', '协奏曲', '通奏低音', '歌剧', '奏鸣曲', '赋格']
        period_scores['baroque'] = sum(1 for kw in baroque_keywords if kw in question_lower)

        # 古典主义
        classical_keywords = ['古典主义', '海顿', '莫扎特', '贝多芬', '华美风格', '敏感风格', '曼海姆', '斯塔米茨', '奏鸣曲式', '交响曲']
        period_scores['classical'] = sum(1 for kw in classical_keywords if kw in question_lower)

        # 浪漫主义
        romantic_keywords = ['浪漫主义', '韦伯', '门德尔松', '舒曼', '肖邦', '李斯特', '瓦格纳', '马勒', '施特劳斯', '情感至上', '标题音乐', '民族主义', '歌剧']
        period_scores['romantic'] = sum(1 for kw in romantic_keywords if kw in question_lower)

        # 20世纪
        twentieth_keywords = ['20世纪', '德彪西', '拉威尔', '勋伯格', '表现主义', '十二音', '新古典主义', '梅西安', '艾夫斯', '印象主义', '简约主义']
        period_scores['twentieth'] = sum(1 for kw in twentieth_keywords if kw in question_lower)
                ...
                if content_length > 0:
                    ...
                    if query:
                        ...
                        response = {
                            ...
                            'knowledge_base_info': f'第{list(knowledge_base.keys()).index(period_key) + 1}章',
                            'ai_generated': False,
                            'model': '用户更新的专业音乐史知识库',
