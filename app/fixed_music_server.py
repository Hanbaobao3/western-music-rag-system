...
def get_available_data_files():
    ...
    if os.path.exists(DATA_DIR):
        # 支持多种文件名格式
        possible_names = [
            'music history.txt',      # 带空格
            'music history 2.txt',    # 带空格
            'music_history.txt',       # 下划线
            'music_history_2.txt',   # 下划线
            'music-history-2.txt',      # 连字符
            'music-history-by-person.txt'  # 之前生成的
        ]
        ...
        for file_name in possible_names:
            ...
            if os.path.exists(file_path):
                ...
                print(f"[DEBUG] 找到数据文件: {file_name}")
...
def load_and_chunk_knowledge():
    ...
    if not available_files:
        print("[WARNING] 未找到任何数据文件，使用默认演示内容")
        return create_demo_knowledge()

    print(f"[INFO] 找到 {len(available_files)} 个数据文件")
    print(f"[INFO] 数据文件: {available_files}")
...
古希腊音乐是西方音乐文明的源头，其理论成就奠定了后世音乐思想的基础。毕达哥拉斯发现了音程的数学比例关系：八度=2:1，纯五度=3:2，纯四度=4:3。柏拉图在《理想国》中认为不同调式具有不同的道德效果。

巴洛克音乐（约1600—1750年）

巴洛克音乐的主要特征包括通奏低音的使用，这是由低音乐器和键盘乐器共同演奏的低声部线条。通奏低音为音乐提供了坚实的基础和即兴的可能性。赋格作为对位法的高级形式在巴洛克时期达到了顶峰。

古典主义时期音乐（约1750—1820年）

古典主义时期确立了奏鸣曲式、交响曲等标准形式。海顿被称为"交响曲之父"，创作了104首交响曲。贝多芬在古典主义晚期创作了最重要的作品，包括九部交响曲。

浪漫主义时期音乐（约1820—1900年）

浪漫主义音乐强调个人情感的表达。肖邦的钢琴作品具有浓厚的民族特色，李斯特发明了交响诗这一体裁。瓦格纳的歌剧改革对后世产生了深远影响。
    """

    demo_lines = demo_content.strip().split('\n')
    chunks = []
...
class AdvancedRAGHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ['/', '/api/health']:
            ...
            response = {
                'status': 'ok',
                'message': '西方音乐史RAG系统运行正常（最终修复版）',
                'server': 'Python http.server',
                'version': '8.0.0',
                'knowledge_base': f'基于{len(available_files)} 个数据文件',
                    ...
                    if query:
                        ...
                        response = {
                            ...
                            'debug_info': {
                                ...
                                'data_files_available': len(get_available_data_files())
                            }
                        }
                        self.wfile.write(json.dumps(response, ensure_ascii=False).encode('utf-8'))
                    else:
...
def run_fixed_server():
    ...
    if not knowledge_chunks:
        print("[ERROR] 知识库为空，服务器仍可运行但使用演示内容")
    else:
        print(f"成功加载 {len(knowledge_chunks)} 个知识块")
